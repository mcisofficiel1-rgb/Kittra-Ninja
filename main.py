...
def check_and_sell_MOLO_NINJA(symbol, price):
    ...
    if gain >= 10.0 and not pos.get('mega_60'):
        vendu=qty_real
        if sell_v19_3(symbol, vendu): CONFIG["stats"]["win"]+=1; CONFIG["stats"]["profit_total"]+= (price-entry)*vendu; CONFIG["coffre_total"]+= price*vendu*0.5; send_tg(f"💥 JACKPOT +10% {symbol}! Coffre {CONFIG['coffre_total']:.2f}$"); del positions[symbol]; save(); return
    if gain >= 6.0 and not pos.get('mega_60'):
        vendu=qty_real*0.99
        if sell_v19_3(symbol, vendu): CONFIG["wallet_perso"]["bonus"]+= price*vendu*0.5; pos['mega_60']=True; pos['mega_50']=True; pos['mega_40']=True; pos['ninja_30']=True; pos['molo_20']=True; pos['molo_10']=True; pos['molo_05']=True; save(); send_tg(f"🔥 +6% {symbol}"); return
    if gain >= 5.0 and not pos.get('mega_50'):
        vendu=qty_real*0.5
        if sell_v19_3(symbol, vendu): CONFIG["coffre_total"]+= price*vendu*0.5; pos['mega_50']=True; pos['mega_40']=True; pos['ninja_30']=True; pos['molo_20']=True; pos['molo_10']=True; pos['molo_05']=True; save(); send_tg(f"💎 +5% {symbol} Coffre {CONFIG['coffre_total']:.2f}$"); return
    if gain >= 4.0 and not pos.get('mega_40'):
        vendu=qty_real*0.4
        if sell_v19_3(symbol, vendu): CONFIG["coffre_total"]+= price*vendu*0.5; pos['mega_40']=True; pos['ninja_30']=True; pos['molo_20']=True; pos['molo_10']=True; pos['molo_05']=True; save(); send_tg(f"🚀 +4% {symbol} Coffre {CONFIG['coffre_total']:.2f}$"); return
    if gain >= 3.0 and not pos.get('ninja_30'):
        vendu=qty_real*0.3
        if sell_v19_3(symbol, vendu): CONFIG["coffre_total"]+= price*vendu*0.5; pos['ninja_30']=True; pos['molo_20']=True; pos['molo_10']=True; pos['molo_05']=True; save(); send_tg(f"🥷 +3% {symbol}"); return
    if gain >= 2.0 and not pos.get('molo_20'):
        vendu=qty_real*0.3
        if sell_v19_3(symbol, vendu): CONFIG["wallet_perso"]["bonus"]+= price*vendu*0.5; pos['molo_20']=True; pos['molo_10']=True; pos['molo_05']=True; save(); send_tg(f"🔥 +2% {symbol}"); return
    if gain >= 1.0 and not pos.get('molo_10'):
        vendu=qty_real*0.3
        if sell_v19_3(symbol, vendu): CONFIG["coffre_total"]+= price*vendu*0.5; pos['molo_10']=True; pos['molo_05']=True; save(); send_tg(f"💰 +1% {symbol} COFFRE {CONFIG['coffre_total']:.2f}$"); return
    if gain >= 0.5 and not pos.get('molo_05'):
        vendu=qty_real*0.3
        if sell_v19_3(symbol, vendu): pos['molo_05']=True; save(); send_tg(f"🐢 +0.5% {symbol} Gain {gain:.2f}%"); return
def trading_loop():
    time.sleep(5); load(); bal=get_real_balance()
    if bal: send_tg(f"🧠 KITTRA V20.5 COFFRE AUTO LIVE! Base 10$ Seuil 11$ | Intel {get_seuil_intelligence_adaptatif(bal+CONFIG['coffre_total'])}% Solde:{bal:.2f}$")
    while True:
        try:
            bal=get_real_balance()
            ...
            if bnb_qty>0.001 and "BNBUSDT" not in positions: positions["BNBUSDT"]={"entry_price":10.34,"qty":bnb_qty,"molo_05":False,"molo_10":False,"molo_20":False,"ninja_30":False,"mega_40":False,"mega_50":False,"mega_60":False}; save(); send_tg(f"🔄 SYNC BNB {bnb_qty}")
...
@app.route("/")
def home():
    bal=get_real_balance(); return f"<h1>V20.5 COFFRE AUTO OK</h1><h2>Solde:{bal} | Pos:{list(positions.keys())} | Coffre:{CONFIG['coffre_total']}$ | Base 10$</h2>"
@app.route("/ping")
def ping(): return jsonify({"v":"20.5","bal":get_real_balance(),"pos":list(positions.keys()),"coffre":CONFIG["coffre_total"]})
