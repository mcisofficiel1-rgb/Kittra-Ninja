...
def load():
    ...
    try:
        with open(KITTRA_FILE,"r") as f: d=json.load(f); positions=d.get("positions",{}); CONFIG["stats"]=d.get("stats",CONFIG["stats"]); CONFIG["coffre_total"]=d.get("coffre",0); CONFIG["wallet_perso"]=d.get("wallet_perso",{"bonus":0})
    except: pass
def save():
...
def transfer_to_fund(amount):
    try:
        ...
        try:
            ...
            return True
        except Exception as e2: print(f"TRANSFER FAIL2 {e2}"); return False
...
def try_buy_safe(s, usdt):
    try:
        if not peut_trader(s, usdt): return False, "skip"
        ...
        if isinstance(res, dict):
            if res.get('retCode')!= 0:
                msg = res.get('retMsg','')
                if is_insufficient_error(res) oimport os, threading, time, requests, json
from datetime import datetime
from flask import Flask, jsonify
from pybit.unified_trading import HTTP
app = Flask(__name__)
...
def continue_trading_existing():
    if not positions:
        for sym in ["BNBUSDT","SOLUSDT","BTCUSDT","ETHUSDT","XRPUSDT"]:
            ...
            if q > 0.0001:
                p = get_price(sym)
                if p: positions[sym]={"entry_price":p,"qty":q,"molo_05":False,"molo_10":False,"molo_20":False,"ninja_30":False,"mega_40":False,"mega_50":False,"mega_60":False}; print(f"🔄 SYNC AUTO TROUVE {sym} qty {q}")
        save()
        ...
        try:
            ...
            if bnb_qty>0.001:
                ...
                if "BNBUSDT" not in positions or positions["BNBUSDT"].get("entry_price",0) < 100:
                    positions["BNBUSDT"]={"entry_price":prix_reel,"qty":bnb_qty,"molo_05":False,"molo_10":False,"molo_20":False,"ninja_30":False,"mega_40":False,"mega_50":False,"mega_60":False}; save(); send_tg(f"🔄 SYNC BNB CORRIGE {bnb_qty} à {prix_reel}$")
            # =======================
            for sym in list(positions.keys()):
                price=get_price(sym)
                if price: check_and_sell_MOLO_NINJA(sym, price)
...
if __name__=="__main__":
    app.run(host="0.0.0.0", port=int(os.getenv("PORT",10000)))￼Enterr is_insufficient_error(msg):
                    print(f"⚠️ SOLDE INSUFFISANT pour {s} ({msg}) -> je continue avec mes positions actuelles {list(positions.keys())}")
                    return False, f"SOLDE_INSUFFISANT_{s}"
                else:
                    print(f"❌ Achat {s} échoué: {msg} -> je passe")
